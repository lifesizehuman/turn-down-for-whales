$(document).ready(function() {
  ...
  function populateMap() {
    ...
    $.ajax({url: queryURL, method: "GET"}).done(function(response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        layer.bindPopup("<p>" +
          "Species: " + response[i].species + "</p>" + "<p>" + "Description: " + response[i].description + "</p>" + "<p>" + "Seen at: " + response[i].latitude + " / " + response[i].longitude + "</p>" + "<p>" + "On: " + response[i].sighted_at + "</p>");
      }
  ...
  $(document).on('click', "#submit", function(event) {
    ...
    database.ref("/search").push({search: searchValue})
  ...
  database.ref("/sightings").on("child_added", function(childSnapshot, prevChildKey) {
    ...
    $('#recent-sighting').on('click', function(event) {
      ...
      function recentPop() {
        ...
        var layer = L.marker([
          empLat, empLong
        ], {icon: myIcon}).addTo(group);
        layer.addTo(group);
        layer.bindPopup("<p>" +
          "Species: " + empSpecies + "</p>" + "<p>" + "Description: " + empDescription + "</p>" + "<p>" + "Seen at: " + empLat + " / " + empLong + "</p>" + "<p>" + "On: " + empTime + " on " + empDate + "</p>");
      }
      recentPop();
    })
  })
